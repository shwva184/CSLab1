...
Nsample<-10000
num_histbreaks<-100
hist(rmajorizing(Nsample),breaks=1000,col="black",xlab="",ylab="",main="HistMajorizing",freq=FALSE, xlim = c(0,50))
...
c = c(1.5,2,2.5)
p = 0.833
t = c(0.75,1.333333,2.083333) # tmin chosen as c^2 / 3
cm = c(max(f(x,1)/nd(x,a,t[1],p)),max(f(x,3)/nd(x,a,t[2],p)),max(f(x,4)/nd(x,a,t[3],p)))
...
```
Mean for c = 1.5 is `r mean[1]`

Mean for c = 2 is `r mean[2]`

Mean for c = 2.5 is `r mean[3]`


Variance for c = 1.5 is `r var[1]`

Variance for c = 2 is `r var[2]`

Variance for c = 2.5 is `r var[3]`


Rejection rate for c = 1.5 is `r rejection_rate[1]`

Rejection rate for c = 2 is `r rejection_rate[2]`
...
Rejection rate also increases as the value of c increases. If we notice the majority constants with respect to c , we can observe that it increases with increase in c.
Majority constant for c = 1.5 is `r cm[1]`. 
Majority constant for c = 2 is `r cm[2]`. 
Majority constant for c = 2.5 is `r cm[2]`. 
As the majority constant increases , the proposal density goes way higher than our target density.
