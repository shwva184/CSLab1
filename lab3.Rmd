...
Since the max(f(x)) is achieved when the value of x is c^2/3, we have chosen tmin value to be the same, because powerlaw is strictly decreasing funtion and maximum of it will be at tmin. Alpha has been chosen by visually deciding which value makes powerlaw function similar to our target function.
...
Nsample<-10000
num_histbreaks<-100
hist(rmajorizing(Nsample),breaks=2000,col="black",xlab="",ylab="",
     main="HistMajorizing",freq=FALSE, xlim = c(0,50))
...
c = c(1.1,1.5,2.5,3)
p = 0.0833
t = c(0.4033333,0.75,2.0833,3) # tmin chosen as c^2 / 3
cm = c(max(f(x,1.1)/nd(x,a,t[1],p)),max(f(x,1.5)/nd(x,a,t[2],p)),
       max(f(x,2.5)/nd(x,a,t[3],p)),max(f(x,3)/nd(x,a,t[4],p)))
mean=var=rejection_rate=c()
for(i in 1:length(c)){
tmin = t[i]
fx_acceptreject<-sapply(rep(c[i],Nsample),accept_reject,c_maj = cm[i], t = tmin)

hist(fx_acceptreject[1,],col="white",border = "black",breaks=10000,xlab="",
     ylab="",freq=FALSE,main="",xlim = c(0,50))
...
Rejection rate for c = 3 is `r rejection_rate[4]`
