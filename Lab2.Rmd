...
```{r}
minusloglikenormal = function(theta, x=data) {
  mu<-theta[1]                   # mean of normal distribution
  sigma2<-theta[2]               # variance of normal distribution
  n = length(x)
  a1 = (n/2)*log(2*pi)-(n/2)*log(sigma2)
  a2 = (sum((x - mu)^2)) / (2 * sigma2^2)
  ans = a1+a2
  return(ans)
}
minusloglikenormal(c(0,1),data)
```

Gradient method

```{r}
gradient = function(theta, x = data) {
  mu<-theta[1]
  sigma2<-theta[2]
  n = length(x)
  dmu = sum(mu - x)/sigma2^2
  dsig = (n/sigma2) - sum((x - mu)^2)/sigma2^3
  gr = c(dmu,dsig)
  return(gr)
}
gradient(c(0,1),data)
...
colnames(df) <- c("CG", "BFGS", "CG with gradient", "BFGS with gradient")
